...
namespace ref {
#define vector_addition ref_vector_addition
#include "vector_addition.cpp"
#undef vector_addition
}
...
static void fill_case(int case_id,
                      ap_int<32> A[VECTOR_SIZE],
                      ap_int<32> B[VECTOR_SIZE],
                      ap_int<32> C[VECTOR_SIZE]) {
// case 0: zero baseline; case 1: discrete patterned values; case 2: smoothly varying values
    for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
        A[i0] = static_cast<ap_int<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
            A[i0] = static_cast<ap_int<32>>(((1 + i0) % 11) - 5);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
            A[i0] = static_cast<ap_int<32>>(((1 * 3 + (i0) * 2 + case_id) % 17) - 8);
        }
    }

    for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
        B[i0] = static_cast<ap_int<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
            B[i0] = static_cast<ap_int<32>>(((2 + i0) % 11) - 5);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
            B[i0] = static_cast<ap_int<32>>(((2 * 3 + (i0) * 2 + case_id) % 17) - 8);
        }
    }

    for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
        C[i0] = static_cast<ap_int<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
            C[i0] = static_cast<ap_int<32>>(((3 + i0) % 11) - 5);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < VECTOR_SIZE; ++i0) {
            C[i0] = static_cast<ap_int<32>>(((3 * 3 + (i0) * 2 + case_id) % 17) - 8);
        }
    }
}

static bool run_case(int case_id) {
    static ap_int<32> dut_A[VECTOR_SIZE] = {};
    static ap_int<32> ref_A[VECTOR_SIZE] = {};
    static ap_int<32> dut_B[VECTOR_SIZE] = {};
    static ap_int<32> ref_B[VECTOR_SIZE] = {};
    static ap_int<32> dut_C[VECTOR_SIZE] = {};
    static ap_int<32> ref_C[VECTOR_SIZE] = {};
    ...
    vector_addition(dut_A, dut_B, dut_C);
    ref::ref_vector_addition(ref_A, ref_B, ref_C);
