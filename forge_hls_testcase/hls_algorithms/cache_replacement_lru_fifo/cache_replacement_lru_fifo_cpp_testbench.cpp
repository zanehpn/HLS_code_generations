...
namespace ref {
#define cache_replacement_lru_fifo ref_cache_replacement_lru_fifo
#include "cache_replacement_lru_fifo.cpp"
#undef cache_replacement_lru_fifo
}
...
// case 0: zero baseline; case 1: discrete patterned values; case 2: smoothly varying values
    for (int i0 = 0; i0 < ADDR_SIZE; ++i0) {
        addresses[i0] = static_cast<ap_uint<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < ADDR_SIZE; ++i0) {
            addresses[i0] = static_cast<ap_uint<32>>((1 + i0) % 251);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < ADDR_SIZE; ++i0) {
            addresses[i0] = static_cast<ap_uint<32>>((1 * 7 + (i0) * 3 + case_id) % 251);
        }
    }

    for (int i0 = 0; i0 < CACHE_SIZE; ++i0) {
        cache[i0] = static_cast<ap_uint<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < CACHE_SIZE; ++i0) {
            cache[i0] = static_cast<ap_uint<32>>((2 + i0) % 251);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < CACHE_SIZE; ++i0) {
            cache[i0] = static_cast<ap_uint<32>>((2 * 7 + (i0) * 3 + case_id) % 251);
        }
    }
    ...
    if (case_id == 1) {
        ...
        is_lru = ((case_id + 1) & 1) != 0;
    }
}

static bool run_case(int case_id) {
    static ap_uint<32> dut_addresses[ADDR_SIZE] = {};
    static ap_uint<32> ref_addresses[ADDR_SIZE] = {};
    static ap_uint<32> dut_cache[CACHE_SIZE] = {};
    static ap_uint<32> ref_cache[CACHE_SIZE] = {};
    static bool dut_is_lru = {};
    static bool ref_is_lru = {};
    ...
    std::memcpy(ref_addresses, dut_addresses, sizeof(dut_addresses));
    std::memcpy(ref_cache, dut_cache, sizeof(dut_cache));
    ref_is_lru = dut_is_lru;

    cache_replacement_lru_fifo(dut_addresses, dut_cache, dut_is_lru);
    ref::ref_cache_replacement_lru_fifo(ref_addresses, ref_cache, ref_is_lru);

    if (!compare_storage(dut_addresses, ref_addresses)) return false;
    if (!compare_storage(dut_cache, ref_cache)) return false;
    if (!compare_storage(dut_is_lru, ref_is_lru)) return false;
    return true;
}
