...
namespace ref {
#define mersenne_twister ref_mersenne_twister
#include "mersenne_twister.cpp"
...
// case 0: zero baseline; case 1: discrete patterned values; case 2: smoothly varying values
    seed = static_cast<uint32_t>(0);
    if (case_id == 1) {
        seed = static_cast<uint32_t>(1 + (case_id));
    } else if (case_id == 2) {
        seed = static_cast<uint32_t>(4 + (case_id + 1));
    }

    for (int i0 = 0; i0 < N; ++i0) {
        output[i0] = static_cast<uint32_t>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < N; ++i0) {
            output[i0] = static_cast<uint32_t>((2 + i0) % 251);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < N; ++i0) {
            output[i0] = static_cast<uint32_t>((2 * 7 + (i0) * 3 + case_id) % 251);
        }
    }
}

static bool run_case(int case_id) {
    static uint32_t dut_seed = {};
    static uint32_t ref_seed = {};
    static uint32_t dut_output[N] = {};
    static uint32_t ref_output[N] = {};
    ...
    mersenne_twister(dut_seed, dut_output);
    ref::ref_mersenne_twister(ref_seed, ref_output);

    if (!compare_storage(dut_seed, ref_seed)) return false;
