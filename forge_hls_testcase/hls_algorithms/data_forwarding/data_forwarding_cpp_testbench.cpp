...
namespace ref {
#define data_forwarding ref_data_forwarding
#include "data_forwarding.cpp"
#undef data_forwarding
}
...
// case 0: zero baseline; case 1: discrete patterned values; case 2: smoothly varying values
    for (int i0 = 0; i0 < SIZE; ++i0) {
        input[i0] = static_cast<ap_int<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < SIZE; ++i0) {
            input[i0] = static_cast<ap_int<32>>(((1 + i0) % 11) - 5);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < SIZE; ++i0) {
            input[i0] = static_cast<ap_int<32>>(((1 * 3 + (i0) * 2 + case_id) % 17) - 8);
        }
    }

    for (int i0 = 0; i0 < SIZE; ++i0) {
        output[i0] = static_cast<ap_int<32>>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < SIZE; ++i0) {
            output[i0] = static_cast<ap_int<32>>(((2 + i0) % 11) - 5);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < SIZE; ++i0) {
            output[i0] = static_cast<ap_int<32>>(((2 * 3 + (i0) * 2 + case_id) % 17) - 8);
        }
    }
}

static bool run_case(int case_id) {
    static ap_int<32> dut_input[SIZE] = {};
    static ap_int<32> ref_input[SIZE] = {};
    static ap_int<32> dut_output[SIZE] = {};
    static ap_int<32> ref_output[SIZE] = {};
    ...
    data_forwarding(dut_input, dut_output);
    ref::ref_data_forwarding(ref_input, ref_output);
