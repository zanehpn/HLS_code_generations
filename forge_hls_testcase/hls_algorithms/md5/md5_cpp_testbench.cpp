...
// case 0: zero baseline; case 1: discrete patterned values; case 2: smoothly varying values
    for (int i0 = 0; i0 < INPUT_SIZE; ++i0) {
        input[i0] = static_cast<uint8_t>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < INPUT_SIZE; ++i0) {
            input[i0] = static_cast<uint8_t>((1 + i0) % 251);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < INPUT_SIZE; ++i0) {
            input[i0] = static_cast<uint8_t>((1 * 7 + (i0) * 3 + case_id) % 251);
        }
    }

    for (int i0 = 0; i0 < 16; ++i0) {
        output[i0] = static_cast<uint8_t>(0);
    }
    if (case_id == 1) {
        for (int i0 = 0; i0 < 16; ++i0) {
            output[i0] = static_cast<uint8_t>((2 + i0) % 251);
        }
    } else if (case_id == 2) {
        for (int i0 = 0; i0 < 16; ++i0) {
            output[i0] = static_cast<uint8_t>((2 * 7 + (i0) * 3 + case_id) % 251);
        }
    }
}

static bool run_case(int case_id) {
    static uint8_t dut_input[INPUT_SIZE] = {};
    static uint8_t ref_input[INPUT_SIZE] = {};
    static uint8_t dut_output[16] = {};
    static uint8_t ref_output[16] = {};
    ...
    md5(dut_input, dut_output);
    ref::ref_md5(ref_input, ref_output);
