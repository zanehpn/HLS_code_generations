...
static ap_int<32> dut_array[ARRAY_SIZE] = {};
static ap_int<32> ref_array[ARRAY_SIZE] = {};
static ap_int<32> case_key = {};
static ap_int<32> dut_index_value = {};
static ap_int<32> ref_index_value = {};
    ap_int<32>* dut_index = &dut_index_value;
    ap_int<32>* ref_index = &ref_index_value;
    seed_value(dut_array, case_id, 1);
    seed_value(case_key, case_id, 2);
    seed_value(dut_index_value, case_id, 3);
    copy_value(ref_array, dut_array);
    copy_value(ref_index_value, dut_index_value);

    binary_search(dut_array, case_key, dut_index);
    ref::binary_search(ref_array, case_key, ref_index);

    if (!compare_storage(dut_array, ref_array)) return 0;
    if (!compare_storage(dut_index_value, ref_index_value)) return 0;
    return 1;
}
