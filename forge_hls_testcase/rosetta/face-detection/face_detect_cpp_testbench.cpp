...
static unsigned char dut_inData[IMAGE_WIDTH] = {};
static unsigned char ref_inData[IMAGE_WIDTH] = {};
static int dut_result_x[RESULT_SIZE] = {};
static int ref_result_x[RESULT_SIZE] = {};
static int dut_result_y[RESULT_SIZE] = {};
static int ref_result_y[RESULT_SIZE] = {};
static int dut_result_w[RESULT_SIZE] = {};
static int ref_result_w[RESULT_SIZE] = {};
static int dut_result_h[RESULT_SIZE] = {};
static int ref_result_h[RESULT_SIZE] = {};
static int dut_result_size_value = {};
static int ref_result_size_value = {};
    int* dut_result_size = &dut_result_size_value;
    int* ref_result_size = &ref_result_size_value;
    seed_value(dut_inData, case_id, 1);
    seed_value(dut_result_x, case_id, 2);
    seed_value(dut_result_y, case_id, 3);
    seed_value(dut_result_w, case_id, 4);
    seed_value(dut_result_h, case_id, 5);
    seed_value(dut_result_size_value, case_id, 6);
    copy_value(ref_inData, dut_inData);
    copy_value(ref_result_x, dut_result_x);
    copy_value(ref_result_y, dut_result_y);
    copy_value(ref_result_w, dut_result_w);
    copy_value(ref_result_h, dut_result_h);
    copy_value(ref_result_size_value, dut_result_size_value);

    face_detect(dut_inData, dut_result_x, dut_result_y, dut_result_w, dut_result_h, dut_result_size);
    ref::face_detect(ref_inData, ref_result_x, ref_result_y, ref_result_w, ref_result_h, ref_result_size);

    if (!compare_storage(dut_inData, ref_inData)) return 0;
    if (!compare_storage(dut_result_x, ref_result_x)) return 0;
    if (!compare_storage(dut_result_y, ref_result_y)) return 0;
    if (!compare_storage(dut_result_w, ref_result_w)) return 0;
    if (!compare_storage(dut_result_h, ref_result_h)) return 0;
    if (!compare_storage(dut_result_size_value, ref_result_size_value)) return 0;
    return 1;
}
