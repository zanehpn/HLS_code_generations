...
#define timer_16bit ref_model_impl
#include "timer_16bit.cpp"
#undef timer_16bit
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/timer_16bit/timer_16bit.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define timer_16bit __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef timer_16bit\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint16_t start_value, uint16_t increment, uint16_t timer[TIMER_SIZE]) { __tb_dut_impl(start_value, increment, timer); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/timer_16bit\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
        ...
        uint16_t ref_start_value;
        uint16_t dut_start_value;
        tb_init_obj(ref_start_value, &seed);
        memcpy(&dut_start_value, &ref_start_value, sizeof(ref_start_value));
        uint16_t ref_increment;
        uint16_t dut_increment;
        tb_init_obj(ref_increment, &seed);
        memcpy(&dut_increment, &ref_increment, sizeof(ref_increment));
      static uint16_t ref_timer[TIMER_SIZE];
      static uint16_t dut_timer[TIMER_SIZE];
        tb_init_obj(ref_timer, &seed);
        memcpy(&dut_timer, &ref_timer, sizeof(ref_timer));
        ref_model_impl(ref_start_value, ref_increment, ref_timer);
        dut_fn(dut_start_value, dut_increment, dut_timer);
        if (!tb_equal_obj(ref_start_value, dut_start_value)) ok = 0;
        if (!tb_equal_obj(ref_increment, dut_increment)) ok = 0;
        if (!tb_equal_obj(ref_timer, dut_timer)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
