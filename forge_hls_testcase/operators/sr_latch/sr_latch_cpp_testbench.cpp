...
#define sr_latch ref_model_impl
#include "sr_latch.cpp"
#undef sr_latch
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/sr_latch/sr_latch.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define sr_latch __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef sr_latch\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_uint<1> S[SIZE], ap_uint<1> R[SIZE], ap_uint<1> Q[SIZE], ap_uint<1> Qn[SIZE]) { __tb_dut_impl(S, R, Q, Qn); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/sr_latch\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_uint<1> ref_S[SIZE];
      static ap_uint<1> dut_S[SIZE];
        tb_init_obj(ref_S, &seed);
        memcpy(&dut_S, &ref_S, sizeof(ref_S));
      static ap_uint<1> ref_R[SIZE];
      static ap_uint<1> dut_R[SIZE];
        tb_init_obj(ref_R, &seed);
        memcpy(&dut_R, &ref_R, sizeof(ref_R));
      static ap_uint<1> ref_Q[SIZE];
      static ap_uint<1> dut_Q[SIZE];
        tb_init_obj(ref_Q, &seed);
        memcpy(&dut_Q, &ref_Q, sizeof(ref_Q));
      static ap_uint<1> ref_Qn[SIZE];
      static ap_uint<1> dut_Qn[SIZE];
        tb_init_obj(ref_Qn, &seed);
        memcpy(&dut_Qn, &ref_Qn, sizeof(ref_Qn));
        ref_model_impl(ref_S, ref_R, ref_Q, ref_Qn);
        dut_fn(dut_S, dut_R, dut_Q, dut_Qn);
        if (!tb_equal_obj(ref_S, dut_S)) ok = 0;
        if (!tb_equal_obj(ref_R, dut_R)) ok = 0;
        if (!tb_equal_obj(ref_Q, dut_Q)) ok = 0;
        if (!tb_equal_obj(ref_Qn, dut_Qn)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
