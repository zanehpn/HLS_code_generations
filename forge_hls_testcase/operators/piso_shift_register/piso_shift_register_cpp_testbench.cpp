...
#define piso_shift_register ref_model_impl
#include "piso_shift_register.cpp"
#undef piso_shift_register
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/piso_shift_register/piso_shift_register.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define piso_shift_register __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef piso_shift_register\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_uint<1> parallel_in[SIZE], ap_uint<1> &serial_out) { __tb_dut_impl(parallel_in, serial_out); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/piso_shift_register\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_uint<1> ref_parallel_in[SIZE];
      static ap_uint<1> dut_parallel_in[SIZE];
        tb_init_obj(ref_parallel_in, &seed);
        memcpy(&dut_parallel_in, &ref_parallel_in, sizeof(ref_parallel_in));
        ap_uint<1> ref_serial_out_obj;
        ap_uint<1> dut_serial_out_obj;
        ap_uint<1> &ref_serial_out = ref_serial_out_obj;
        ap_uint<1> &dut_serial_out = dut_serial_out_obj;
        tb_init_obj(ref_serial_out_obj, &seed);
        memcpy(&dut_serial_out_obj, &ref_serial_out_obj, sizeof(ref_serial_out_obj));
        ref_model_impl(ref_parallel_in, ref_serial_out);
        dut_fn(dut_parallel_in, dut_serial_out);
        if (!tb_equal_obj(ref_parallel_in, dut_parallel_in)) ok = 0;
        if (!tb_equal_obj(ref_serial_out_obj, dut_serial_out_obj)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
