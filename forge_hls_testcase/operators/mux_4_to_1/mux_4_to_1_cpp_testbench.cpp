...
#define mux_4_to_1 ref_model_impl
#include "mux_4_to_1.cpp"
#undef mux_4_to_1
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/mux_4_to_1/mux_4_to_1.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define mux_4_to_1 __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef mux_4_to_1\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_uint<2> sel[ARRAY_SIZE], ap_uint<8> in0[ARRAY_SIZE], ap_uint<8> in1[ARRAY_SIZE], ap_uint<8> in2[ARRAY_SIZE], ap_uint<8> in3[ARRAY_SIZE], ap_uint<8> out[ARRAY_SIZE]) { __tb_dut_impl(sel, in0, in1, in2, in3, out); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/mux_4_to_1\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_uint<2> ref_sel[ARRAY_SIZE];
      static ap_uint<2> dut_sel[ARRAY_SIZE];
        tb_init_obj(ref_sel, &seed);
        memcpy(&dut_sel, &ref_sel, sizeof(ref_sel));
      static ap_uint<8> ref_in0[ARRAY_SIZE];
      static ap_uint<8> dut_in0[ARRAY_SIZE];
        tb_init_obj(ref_in0, &seed);
        memcpy(&dut_in0, &ref_in0, sizeof(ref_in0));
      static ap_uint<8> ref_in1[ARRAY_SIZE];
      static ap_uint<8> dut_in1[ARRAY_SIZE];
        tb_init_obj(ref_in1, &seed);
        memcpy(&dut_in1, &ref_in1, sizeof(ref_in1));
      static ap_uint<8> ref_in2[ARRAY_SIZE];
      static ap_uint<8> dut_in2[ARRAY_SIZE];
        tb_init_obj(ref_in2, &seed);
        memcpy(&dut_in2, &ref_in2, sizeof(ref_in2));
      static ap_uint<8> ref_in3[ARRAY_SIZE];
      static ap_uint<8> dut_in3[ARRAY_SIZE];
        tb_init_obj(ref_in3, &seed);
        memcpy(&dut_in3, &ref_in3, sizeof(ref_in3));
      static ap_uint<8> ref_out[ARRAY_SIZE];
      static ap_uint<8> dut_out[ARRAY_SIZE];
        tb_init_obj(ref_out, &seed);
        memcpy(&dut_out, &ref_out, sizeof(ref_out));
        ref_model_impl(ref_sel, ref_in0, ref_in1, ref_in2, ref_in3, ref_out);
        dut_fn(dut_sel, dut_in0, dut_in1, dut_in2, dut_in3, dut_out);
        if (!tb_equal_obj(ref_sel, dut_sel)) ok = 0;
        if (!tb_equal_obj(ref_in0, dut_in0)) ok = 0;
        if (!tb_equal_obj(ref_in1, dut_in1)) ok = 0;
        if (!tb_equal_obj(ref_in2, dut_in2)) ok = 0;
        if (!tb_equal_obj(ref_in3, dut_in3)) ok = 0;
        if (!tb_equal_obj(ref_out, dut_out)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
