...
#define multiplier_32bit ref_model_impl
#include "multiplier_32bit.cpp"
#undef multiplier_32bit
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/multiplier_32bit/multiplier_32bit.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define multiplier_32bit __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef multiplier_32bit\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint32_t A[ARRAY_SIZE], uint32_t B[ARRAY_SIZE], uint32_t C[ARRAY_SIZE]) { __tb_dut_impl(A, B, C); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/multiplier_32bit\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint32_t ref_A[ARRAY_SIZE];
      static uint32_t dut_A[ARRAY_SIZE];
        tb_init_obj(ref_A, &seed);
        memcpy(&dut_A, &ref_A, sizeof(ref_A));
      static uint32_t ref_B[ARRAY_SIZE];
      static uint32_t dut_B[ARRAY_SIZE];
        tb_init_obj(ref_B, &seed);
        memcpy(&dut_B, &ref_B, sizeof(ref_B));
      static uint32_t ref_C[ARRAY_SIZE];
      static uint32_t dut_C[ARRAY_SIZE];
        tb_init_obj(ref_C, &seed);
        memcpy(&dut_C, &ref_C, sizeof(ref_C));
        ref_model_impl(ref_A, ref_B, ref_C);
        dut_fn(dut_A, dut_B, dut_C);
        if (!tb_equal_obj(ref_A, dut_A)) ok = 0;
        if (!tb_equal_obj(ref_B, dut_B)) ok = 0;
        if (!tb_equal_obj(ref_C, dut_C)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
