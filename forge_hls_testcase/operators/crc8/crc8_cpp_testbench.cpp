...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/crc8/crc8.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define crc8 __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef crc8\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint8_t data[DATA_SIZE], uint8_t* crc_out) { __tb_dut_impl(data, crc_out); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/crc8\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint8_t ref_data[DATA_SIZE];
      static uint8_t dut_data[DATA_SIZE];
        tb_init_obj(ref_data, &seed);
        memcpy(&dut_data, &ref_data, sizeof(ref_data));
      static unsigned char ref_crc_out_storage[TB_PTR_BYTES];
      static unsigned char dut_crc_out_storage[TB_PTR_BYTES];
        uint8_t* ref_crc_out;
        uint8_t* dut_crc_out;
        ref_crc_out = (__typeof__(ref_crc_out))(ref_crc_out_storage);
        dut_crc_out = (__typeof__(dut_crc_out))(dut_crc_out_storage);
        tb_fill(ref_crc_out_storage, sizeof(ref_crc_out_storage), &seed);
        memcpy(dut_crc_out_storage, ref_crc_out_storage, sizeof(ref_crc_out_storage));
        ref_model_impl(ref_data, ref_crc_out);
        dut_fn(dut_data, dut_crc_out);
        if (!tb_equal_obj(ref_data, dut_data)) ok = 0;
        if (memcmp(ref_crc_out_storage, dut_crc_out_storage, sizeof(ref_crc_out_storage)) != 0) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
