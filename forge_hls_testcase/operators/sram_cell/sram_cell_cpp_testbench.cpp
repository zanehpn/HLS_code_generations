...
#define sram_cell ref_model_impl
#include "sram_cell.cpp"
#undef sram_cell
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/sram_cell/sram_cell.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define sram_cell __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef sram_cell\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint16_t input_array[ARRAY_SIZE], uint16_t output_array[ARRAY_SIZE]) { __tb_dut_impl(input_array, output_array); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/sram_cell\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint16_t ref_input_array[ARRAY_SIZE];
      static uint16_t dut_input_array[ARRAY_SIZE];
        tb_init_obj(ref_input_array, &seed);
        memcpy(&dut_input_array, &ref_input_array, sizeof(ref_input_array));
      static uint16_t ref_output_array[ARRAY_SIZE];
      static uint16_t dut_output_array[ARRAY_SIZE];
        tb_init_obj(ref_output_array, &seed);
        memcpy(&dut_output_array, &ref_output_array, sizeof(ref_output_array));
        ref_model_impl(ref_input_array, ref_output_array);
        dut_fn(dut_input_array, dut_output_array);
        if (!tb_equal_obj(ref_input_array, dut_input_array)) ok = 0;
        if (!tb_equal_obj(ref_output_array, dut_output_array)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
