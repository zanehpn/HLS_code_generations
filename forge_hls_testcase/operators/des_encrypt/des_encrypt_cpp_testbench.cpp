...
#define des_encrypt ref_model_impl
#include "des_encrypt.cpp"
#undef des_encrypt
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/des_encrypt/des_encrypt.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define des_encrypt __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef des_encrypt\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint64_t input[DATA_SIZE], uint64_t output[DATA_SIZE], uint64_t key) { __tb_dut_impl(input, output, key); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/des_encrypt\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint64_t ref_input[DATA_SIZE];
      static uint64_t dut_input[DATA_SIZE];
        tb_init_obj(ref_input, &seed);
        memcpy(&dut_input, &ref_input, sizeof(ref_input));
      static uint64_t ref_output[DATA_SIZE];
      static uint64_t dut_output[DATA_SIZE];
        tb_init_obj(ref_output, &seed);
        memcpy(&dut_output, &ref_output, sizeof(ref_output));
        uint64_t ref_key;
        uint64_t dut_key;
        tb_init_obj(ref_key, &seed);
        memcpy(&dut_key, &ref_key, sizeof(ref_key));
        ref_model_impl(ref_input, ref_output, ref_key);
        dut_fn(dut_input, dut_output, dut_key);
        if (!tb_equal_obj(ref_input, dut_input)) ok = 0;
        if (!tb_equal_obj(ref_output, dut_output)) ok = 0;
        if (!tb_equal_obj(ref_key, dut_key)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
