...
#define nand_gate ref_model_impl
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/nand_gate/nand_gate.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define nand_gate __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef nand_gate\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_uint<1> A[SIZE], ap_uint<1> B[SIZE], ap_uint<1> C[SIZE]) { __tb_dut_impl(A, B, C); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/nand_gate\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_uint<1> ref_A[SIZE];
      static ap_uint<1> dut_A[SIZE];
        tb_init_obj(ref_A, &seed);
        memcpy(&dut_A, &ref_A, sizeof(ref_A));
      static ap_uint<1> ref_B[SIZE];
      static ap_uint<1> dut_B[SIZE];
        tb_init_obj(ref_B, &seed);
        memcpy(&dut_B, &ref_B, sizeof(ref_B));
      static ap_uint<1> ref_C[SIZE];
      static ap_uint<1> dut_C[SIZE];
