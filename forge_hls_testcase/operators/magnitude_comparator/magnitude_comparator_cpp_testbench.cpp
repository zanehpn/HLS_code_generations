...
#define magnitude_comparator ref_model_impl
#include "magnitude_comparator.cpp"
#undef magnitude_comparator
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/magnitude_comparator/magnitude_comparator.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define magnitude_comparator __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef magnitude_comparator\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_int<32> input1[ARRAY_SIZE], ap_int<32> input2[ARRAY_SIZE], ap_int<1> output[ARRAY_SIZE]) { __tb_dut_impl(input1, input2, output); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/magnitude_comparator\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_int<32> ref_input1[ARRAY_SIZE];
      static ap_int<32> dut_input1[ARRAY_SIZE];
        tb_init_obj(ref_input1, &seed);
        memcpy(&dut_input1, &ref_input1, sizeof(ref_input1));
      static ap_int<32> ref_input2[ARRAY_SIZE];
      static ap_int<32> dut_input2[ARRAY_SIZE];
        tb_init_obj(ref_input2, &seed);
        memcpy(&dut_input2, &ref_input2, sizeof(ref_input2));
      static ap_int<1> ref_output[ARRAY_SIZE];
      static ap_int<1> dut_output[ARRAY_SIZE];
        tb_init_obj(ref_output, &seed);
        memcpy(&dut_output, &ref_output, sizeof(ref_output));
        ref_model_impl(ref_input1, ref_input2, ref_output);
        dut_fn(dut_input1, dut_input2, dut_output);
