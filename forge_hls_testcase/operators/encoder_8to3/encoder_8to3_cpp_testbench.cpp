...
#define encoder_8to3 ref_model_impl
#include "encoder_8to3.cpp"
#undef encoder_8to3
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/encoder_8to3/encoder_8to3.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define encoder_8to3 __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef encoder_8to3\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_uint<8> input[INPUT_SIZE], ap_uint<3> output[INPUT_SIZE]) { __tb_dut_impl(input, output); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/encoder_8to3\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_uint<8> ref_input[INPUT_SIZE];
      static ap_uint<8> dut_input[INPUT_SIZE];
        tb_init_obj(ref_input, &seed);
        memcpy(&dut_input, &ref_input, sizeof(ref_input));
      static ap_uint<3> ref_output[INPUT_SIZE];
      static ap_uint<3> dut_output[INPUT_SIZE];
