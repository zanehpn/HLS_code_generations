...
#define adder_16bit ref_model_impl
#include "adder_16bit.cpp"
#undef adder_16bit
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/adder_16bit/adder_16bit.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define adder_16bit __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef adder_16bit\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint16_t A[ARRAY_SIZE], uint16_t B[ARRAY_SIZE], uint16_t C[ARRAY_SIZE]) { __tb_dut_impl(A, B, C); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/adder_16bit\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint16_t ref_A[ARRAY_SIZE];
      static uint16_t dut_A[ARRAY_SIZE];
        tb_init_obj(ref_A, &seed);
        memcpy(&dut_A, &ref_A, sizeof(ref_A));
      static uint16_t ref_B[ARRAY_SIZE];
      static uint16_t dut_B[ARRAY_SIZE];
        tb_init_obj(ref_B, &seed);
        memcpy(&dut_B, &ref_B, sizeof(ref_B));
      static uint16_t ref_C[ARRAY_SIZE];
      static uint16_t dut_C[ARRAY_SIZE];
