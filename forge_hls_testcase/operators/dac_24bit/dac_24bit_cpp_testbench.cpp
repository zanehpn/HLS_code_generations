...
#define dac_24bit ref_model_impl
#include "dac_24bit.cpp"
#undef dac_24bit
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/dac_24bit/dac_24bit.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define dac_24bit __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef dac_24bit\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint32_t input[ARRAY_SIZE], uint8_t output[ARRAY_SIZE]) { __tb_dut_impl(input, output); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/dac_24bit\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint32_t ref_input[ARRAY_SIZE];
      static uint32_t dut_input[ARRAY_SIZE];
        tb_init_obj(ref_input, &seed);
        memcpy(&dut_input, &ref_input, sizeof(ref_input));
      static uint8_t ref_output[ARRAY_SIZE];
      static uint8_t dut_output[ARRAY_SIZE];
