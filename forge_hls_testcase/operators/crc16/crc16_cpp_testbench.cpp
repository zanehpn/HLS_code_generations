...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/crc16/crc16.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define crc16 __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef crc16\n");
    fprintf(wf, "extern \"C\" void __tb_entry(uint16_t data[ARRAY_SIZE], uint16_t crc[ARRAY_SIZE]) { __tb_dut_impl(data, crc); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/crc16\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static uint16_t ref_crc[ARRAY_SIZE];
      static uint16_t dut_crc[ARRAY_SIZE];
        tb_init_obj(ref_crc, &seed);
        memcpy(&dut_crc, &ref_crc, sizeof(ref_crc));
        ref_model_impl(ref_data, ref_crc);
        dut_fn(dut_data, dut_crc);
        if (!tb_equal_obj(ref_data, dut_data)) ok = 0;
        if (!tb_equal_obj(ref_crc, dut_crc)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
