...
#define comparator_8bit ref_model_impl
#include "comparator_8bit.cpp"
#undef comparator_8bit
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/operators/comparator_8bit/comparator_8bit.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define comparator_8bit __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef comparator_8bit\n");
    fprintf(wf, "extern \"C\" void __tb_entry(ap_uint<8> input1[ARRAY_SIZE], ap_uint<8> input2[ARRAY_SIZE], bool result[ARRAY_SIZE]) { __tb_dut_impl(input1, input2, result); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/operators/comparator_8bit\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int it = 0; it < TB_ITERS && ok; ++it) {
      ...
      static ap_uint<8> ref_input1[ARRAY_SIZE];
      static ap_uint<8> dut_input1[ARRAY_SIZE];
        tb_init_obj(ref_input1, &seed);
        memcpy(&dut_input1, &ref_input1, sizeof(ref_input1));
      static ap_uint<8> ref_input2[ARRAY_SIZE];
      static ap_uint<8> dut_input2[ARRAY_SIZE];
        tb_init_obj(ref_input2, &seed);
        memcpy(&dut_input2, &ref_input2, sizeof(ref_input2));
      static bool ref_result[ARRAY_SIZE];
      static bool dut_result[ARRAY_SIZE];
        tb_init_obj(ref_result, &seed);
        memcpy(&dut_result, &ref_result, sizeof(ref_result));
        ref_model_impl(ref_input1, ref_input2, ref_result);
        dut_fn(dut_input1, dut_input2, dut_result);
        if (!tb_equal_obj(ref_input1, dut_input1)) ok = 0;
        if (!tb_equal_obj(ref_input2, dut_input2)) ok = 0;
        if (!tb_equal_obj(ref_result, dut_result)) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
