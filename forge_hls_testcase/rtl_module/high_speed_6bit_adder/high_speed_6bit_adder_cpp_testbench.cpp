...
#include "high_speed_6bit_adder.cpp"
...
static ap_uint<6> dut_A[ARRAY_SIZE] = {};
static ap_uint<6> ref_A[ARRAY_SIZE] = {};
static ap_uint<6> dut_B[ARRAY_SIZE] = {};
static ap_uint<6> ref_B[ARRAY_SIZE] = {};
static ap_uint<6> dut_C[ARRAY_SIZE] = {};
static ap_uint<6> ref_C[ARRAY_SIZE] = {};
    seed_value(dut_A, case_id, 1);
    seed_value(dut_B, case_id, 2);
    seed_value(dut_C, case_id, 3);
    copy_value(ref_A, dut_A);
    copy_value(ref_B, dut_B);
    copy_value(ref_C, dut_C);

    high_speed_6bit_adder(dut_A, dut_B, dut_C);
    ref::high_speed_6bit_adder(ref_A, ref_B, ref_C);

    if (!compare_storage(dut_A, ref_A)) return 0;
    if (!compare_storage(dut_B, ref_B)) return 0;
    if (!compare_storage(dut_C, ref_C)) return 0;
    return 1;
}
