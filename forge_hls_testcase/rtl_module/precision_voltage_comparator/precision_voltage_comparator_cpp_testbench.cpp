...
#include "precision_voltage_comparator.cpp"
...
static ap_int<16> dut_voltage1[ARRAY_SIZE] = {};
static ap_int<16> ref_voltage1[ARRAY_SIZE] = {};
static ap_int<16> dut_voltage2[ARRAY_SIZE] = {};
static ap_int<16> ref_voltage2[ARRAY_SIZE] = {};
static ap_int<1> dut_result[ARRAY_SIZE] = {};
static ap_int<1> ref_result[ARRAY_SIZE] = {};
    seed_value(dut_voltage1, case_id, 1);
    seed_value(dut_voltage2, case_id, 2);
    seed_value(dut_result, case_id, 3);
    copy_value(ref_voltage1, dut_voltage1);
    copy_value(ref_voltage2, dut_voltage2);
    copy_value(ref_result, dut_result);

    precision_voltage_comparator(dut_voltage1, dut_voltage2, dut_result);
    ref::precision_voltage_comparator(ref_voltage1, ref_voltage2, ref_result);

    if (!compare_storage(dut_voltage1, ref_voltage1)) return 0;
    if (!compare_storage(dut_voltage2, ref_voltage2)) return 0;
    if (!compare_storage(dut_result, ref_result)) return 0;
    return 1;
}
