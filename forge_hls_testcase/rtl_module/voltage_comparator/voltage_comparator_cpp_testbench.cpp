...
#include "voltage_comparator.cpp"
...
static ap_fixed<16, 8> dut_input_voltage[ARRAY_SIZE] = {};
static ap_fixed<16, 8> ref_input_voltage[ARRAY_SIZE] = {};
static ap_fixed<16, 8> case_reference_voltage = {};
static bool dut_output_result[ARRAY_SIZE] = {};
static bool ref_output_result[ARRAY_SIZE] = {};
    seed_value(dut_input_voltage, case_id, 1);
    seed_value(case_reference_voltage, case_id, 2);
    seed_value(dut_output_result, case_id, 3);
    copy_value(ref_input_voltage, dut_input_voltage);
    copy_value(ref_output_result, dut_output_result);

    voltage_comparator(dut_input_voltage, case_reference_voltage, dut_output_result);
    ref::voltage_comparator(ref_input_voltage, case_reference_voltage, ref_output_result);

    if (!compare_storage(dut_input_voltage, ref_input_voltage)) return 0;
    if (!compare_storage(dut_output_result, ref_output_result)) return 0;
    return 1;
}
