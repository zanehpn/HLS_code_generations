...
#include "binary_rate_multiplier.cpp"
...
static ap_uint<1> dut_input[N] = {};
static ap_uint<1> ref_input[N] = {};
static ap_uint<1> dut_rate[N] = {};
static ap_uint<1> ref_rate[N] = {};
static ap_uint<1> dut_output[N] = {};
static ap_uint<1> ref_output[N] = {};
    seed_value(dut_input, case_id, 1);
    seed_value(dut_rate, case_id, 2);
    seed_value(dut_output, case_id, 3);
    copy_value(ref_input, dut_input);
    copy_value(ref_rate, dut_rate);
    copy_value(ref_output, dut_output);

    binary_rate_multiplier(dut_input, dut_rate, dut_output);
    ref::binary_rate_multiplier(ref_input, ref_rate, ref_output);

    if (!compare_storage(dut_input, ref_input)) return 0;
    if (!compare_storage(dut_rate, ref_rate)) return 0;
