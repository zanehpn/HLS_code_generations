...
#include "peripheral_interface.cpp"
...
static ap_int<32> dut_input_data[ARRAY_SIZE] = {};
static ap_int<32> ref_input_data[ARRAY_SIZE] = {};
static ap_int<32> dut_output_data[ARRAY_SIZE] = {};
static ap_int<32> ref_output_data[ARRAY_SIZE] = {};
    seed_value(dut_input_data, case_id, 1);
    seed_value(dut_output_data, case_id, 2);
    copy_value(ref_input_data, dut_input_data);
    copy_value(ref_output_data, dut_output_data);

    peripheral_interface(dut_input_data, dut_output_data);
    ref::peripheral_interface(ref_input_data, ref_output_data);

    if (!compare_storage(dut_input_data, ref_input_data)) return 0;
    if (!compare_storage(dut_output_data, ref_output_data)) return 0;
    return 1;
}
