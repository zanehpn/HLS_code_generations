...
static ap_int<32> dut_steps[ARRAY_SIZE] = {};
static ap_int<32> ref_steps[ARRAY_SIZE] = {};
static ap_int<32> case_threshold = {};
static ap_int<32> dut_count = {};
static ap_int<32> ref_count = {};
    seed_value(dut_steps, case_id, 1);
    seed_value(case_threshold, case_id, 2);
    seed_value(dut_count, case_id, 3);
    copy_value(ref_steps, dut_steps);
    copy_value(ref_count, dut_count);

    step_counter(dut_steps, case_threshold, dut_count);
    ref::step_counter(ref_steps, case_threshold, ref_count);

    if (!compare_storage(dut_steps, ref_steps)) return 0;
    if (!compare_storage(dut_count, ref_count)) return 0;
    return 1;
}
