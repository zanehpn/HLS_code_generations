...
#define loop_perfect ref_model_impl
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/perfect_loop/combined.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define loop_perfect __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef loop_perfect\n");
    fprintf(wf, "extern \"C\" void __tb_entry(din_t A[N], dout_t B[N]) { __tb_dut_impl(A, B); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/perfect_loop\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int case_id = 0; case_id < TB_CASES && ok; ++case_id) {
      static din_t ref_A[N];
      static din_t dut_A[N];
        tb_init_obj(ref_A, case_id, 0u);
        memcpy(&dut_A, &ref_A, sizeof(ref_A));
      static dout_t ref_B[N];
      static dout_t dut_B[N];
        tb_init_obj(ref_B, case_id, 1u);
        memcpy(&dut_B, &ref_B, sizeof(ref_B));
        ref_model_impl(ref_A, ref_B);
        dut_fn(dut_A, dut_B);
