...
#define loop_pipeline ref_model_impl
...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/pipelined_loop/combined.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define loop_pipeline __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef loop_pipeline\n");
    fprintf(wf, "extern \"C\" dout_t __tb_entry(din_t A[N]) { return __tb_dut_impl(A); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/pipelined_loop\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int case_id = 0; case_id < TB_CASES && ok; ++case_id) {
      ...
      static din_t dut_A[N];
        ...
        dout_t ref_ret = ref_model_impl(ref_A);
        dout_t dut_ret = dut_fn(dut_A);
