...
int main(int argc, char** argv) {
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/aggregation_of_m_axi_ports/combined.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define dut __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef dut\n");
    fprintf(wf, "extern \"C\" int __tb_entry(A* arr) { return __tb_dut_impl(arr); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/aggregation_of_m_axi_ports\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int case_id = 0; case_id < TB_CASES && ok; ++case_id) {
      static unsigned char ref_arr_storage[TB_PTR_BYTES];
      static unsigned char dut_arr_storage[TB_PTR_BYTES];
        A* ref_arr;
        A* dut_arr;
        ref_arr = (__typeof__(ref_arr))(ref_arr_storage);
        dut_arr = (__typeof__(dut_arr))(dut_arr_storage);
        tb_fill(ref_arr_storage, sizeof(ref_arr_storage), case_id, 0u);
        memcpy(dut_arr_storage, ref_arr_storage, sizeof(ref_arr_storage));
        int ref_ret = ref_model_impl(ref_arr);
        int dut_ret = dut_fn(dut_arr);
        if (!tb_equal_obj(ref_ret, dut_ret)) ok = 0;
        if (memcmp(ref_arr_storage, dut_arr_storage, sizeof(ref_arr_storage)) != 0) ok = 0;
    }
    dlclose(handle);
    remove(wrap_path);
    remove(so_path);
