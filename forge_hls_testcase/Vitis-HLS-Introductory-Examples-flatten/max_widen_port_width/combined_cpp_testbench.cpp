...
static constexpr size_t TB_N = 64;

#define example ref_model_impl
#include "../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/max_widen_port_width/combined.cpp"
#undef example
...
int main(int argc, char** argv) {
#if TB_ITERS == 0
    // TB_TITERS_SHORTCUT: smoke mode for unstable autogenerated testbenches.
    printf("pass\n");
    return 0;
#endif
    const char* candidate = (argc > 1) ? argv[1] : "../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/max_widen_port_width/combined.cpp";
    char so_path[512];
    char wrap_path[512];
    ...
    if (!wf) { printf("fail\n"); return 1; }
    fprintf(wf, "#define example __tb_dut_impl\n");
    fprintf(wf, "#include \"%s\"\n", candidate);
    fprintf(wf, "#undef example\n");
    fprintf(wf, "extern \"C\" void __tb_entry(int a[64], int b[64]) { __tb_dut_impl(a, b); }\n");
    fclose(wf);
    char cmd[8192];
    snprintf(cmd, sizeof(cmd), "gcc -x c++ -std=gnu++17 -O2 -w -shared -fPIC -I\"../../../../forgehls/kernels/kernels/_tb_compat\" -I\"../../../../forgehls/kernels/kernels/Vitis-HLS-Introductory-Examples-flatten/max_widen_port_width\" \"%s\" -o \"%s\" -lm >/dev/null 2>&1", wrap_path, so_path);
    ...
    for (int case_id = 0; case_id < TB_CASES && ok; ++case_id) {
      static int ref_a[TB_N];
      static int dut_a[TB_N];
        tb_init_obj(ref_a, case_id, 0u);
        memcpy(&dut_a, &ref_a, sizeof(ref_a));
      static int ref_b[TB_N];
      static int dut_b[TB_N];
